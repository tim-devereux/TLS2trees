...
RUN apt-get update && \
    apt-get install -y apt-utils git curl vim unzip wget build-essential python3-pip cuda-compat-11-8 && \
    apt-get clean && \
...
RUN ln -s /usr/local/cuda-11.8/lib64/libcusolver.so.11 /usr/local/cuda-11.8/lib64/libcusolver.so.10 && \
    chmod a+x /opt/tls2trees/semantic.py /opt/tls2trees/instance.py

# Set environmental variables
ENV LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/usr/local/cuda-11.8/lib:/usr/local/cuda-11.8/lib64:/usr/local/cuda/compat
